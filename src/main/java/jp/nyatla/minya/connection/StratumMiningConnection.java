...
import jp.nyatla.minya.stratum.*;
...
public class StratumMiningConnection implements IMiningConnection {
    private final String CLIENT_NAME = "cpuminer-multi/1.3.3";
    private final Object _data_lock = new Object();
    ...
    public MiningWork connect() throws MinyaException {
        ...
        try {
            ...
            for (i = 0; i < 3; i++) {
                MinyaLog.message("Request Stratum authorise...");
                StratumJsonResultStandard auth = (StratumJsonResultStandard) this._rx_thread.waitForJsonResult(this._sock.login(this._uid, this._pass), StratumJsonResultStandard.class, 3000);
    ...
    public void submitWork(MiningWork i_work, int i_nonce) throws MinyaException {
        ...
        String ntime = w.data.getStr(MiningWork.INDEX_OF_NTIME, 4);
        //Stratum送信
        try {
            long id = this._sock.submit(this._uid, w.job_id, w.xnonce2, w.xnonce2);
